...
export class Rotary extends CanvasElement {
	...
	constructor(
		...
		this.scaleFactor = this.dimensions.minDim * 0.6
		this.centerY = this.centerY + 0.05 * this.scaleFactor
	...
	render(): void {
		...
		this.context.font = this.dimensions.minDim * 0.15 + 'px system-ui'
		this.fillText(0, -0.6, this.options.label)

		this.context.font = this.dimensions.minDim * 0.08 + 'px system-ui'
		this.fillText(0, -0.4, this.options.subtitle ?? '')

		this.setLineWidth(0.03)
		this.context.font = this.dimensions.minDim * 0.12 + 'px system-ui'
