...
export class Toggle extends CanvasElement {
	...
	constructor(
		...
		this.centerY = this.centerY + 0.5 * this.scaleFactor
	...
	override render() {
		...
		const mainLabelSize = this.dimensions.minDim * 0.15 + 'px'
		this.context.font = mainLabelSize + ' system-ui'
		this.fillText(0, -2.7, this.options.label.toLocaleUpperCase())

		const optionLabelSize = this.dimensions.minDim * 0.08 + 'px'
