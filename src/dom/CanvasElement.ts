...
export const drawZoneForElement = (el: HTMLElement): CanvasDimensions => {
	const dpr = findDPR()
	const dz = {
	...
	return {
		...dz,
		bottom: dz.top + dz.height,
		right: dz.left + dz.width,
		centerX: dz.left + dz.width / 2,
		centerY: dz.top + dz.height / 2,
		minDim: Math.min(dz.height, dz.width),
	}
}
...
export abstract class CanvasElement implements Disposable {
	...
	protected traceCircle(r: number, x: number = 0, y: number = 0) {
		...
		return (l: Location) => {
			return (
				(l.x - canvasX) ** 2 + (l.y - canvasY) ** 2 <
				(this.scaleFactor * r) ** 2
			)
		}
	}
